        ...
        if self._event_disposition_service is not None:
            ...
            try:
                ...
                if not terminal_verify_ready:
                    ...
                    need_manual = True
                    # Verdict-gated skip is an analyst hold, not a writeback
                    # failure. Stuffing the deferred action into
                    # blocked_writebacks would freeze resume until a
                    # writeback resolve that can never succeed.
                    if activate_result.skipped_reason != "verdict_not_terminal":
