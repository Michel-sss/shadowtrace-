...
describe("EventAuditPanel degradation and realtime", () => {
  ...
  it("appends started/completed socket calls to the event audit", async () => {
    ...
    renderWithProviders(<EventAuditPanel eventId="evt-73" />);
    await screen.findByText("暂无工具调用记录");
    ...
    expect((await screen.findAllByText("query_dns")).length).toBeGreaterThan(0);
    ...
    await waitFor(() => expect(screen.getByText("Action success")).toBeInTheDocument());
  });
});
